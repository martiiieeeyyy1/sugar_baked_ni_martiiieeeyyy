<?php
// ===================================================
// cashier_dashboard.php - Sugar Baked Cashier Dashboard
// ===================================================
...
// ===== FETCH DASHBOARD STATISTICS (COMPLETED SALES ONLY - CASHIER SPECIFIC) =====

// Total Revenue (Completed Sales - This Cashier Only)
$revenueQuery = $conn->prepare("
    SELECT SUM(price * quantity * 1.12) AS total_revenue 
    FROM sales 
    WHERE status = 'Completed' AND cashier_id = ?
");
$revenueQuery->bind_param("i", $cashier_id);
$revenueQuery->execute();
$totalRevenue = $revenueQuery->get_result()->fetch_assoc()['total_revenue'] ?? 0;
$revenueQuery->close();

// Total Completed Sales (This Cashier Only)
$completedSalesQuery = $conn->prepare("
    SELECT COUNT(*) AS total_sales 
    FROM sales 
    WHERE status = 'Completed' AND cashier_id = ?
");
$completedSalesQuery->bind_param("i", $cashier_id);
$completedSalesQuery->execute();
$totalCompletedSales = $completedSalesQuery->get_result()->fetch_assoc()['total_sales'] ?? 0;
$completedSalesQuery->close();
...
// ===== BESTSELLING PRODUCTS (TOP 5 - COMPLETED SALES - THIS CASHIER ONLY) =====
$bestsellingQuery = $conn->prepare("
    ...
    LIMIT 5
");
$bestsellingQuery->bind_param("i", $cashier_id);
$bestsellingQuery->execute();
$bestsellingResult = $bestsellingQuery->get_result();

// ===== RECENT COMPLETED SALES (LAST 5 - THIS CASHIER ONLY) =====
$recentSalesQuery = $conn->prepare("
    SELECT 
        product_name, 
        quantity, 
        price, 
    ...
    LIMIT 5
");
$recentSalesQuery->bind_param("i", $cashier_id);
$recentSalesQuery->execute();
$recentSalesResult = $recentSalesQuery->get_result();

// ===== DAILY SALES CHART DATA (LAST 7 DAYS - COMPLETED ONLY - THIS CASHIER) =====
$dailySalesData = [];
for ($i = 6; $i >= 0; $i--) {
    ...
    $salesQuery = $conn->prepare("
        ...
        FROM sales 
        WHERE DATE(sale_date) = ? AND status = 'Completed' AND cashier_id = ?
    ");
    $salesQuery->bind_param("si", $date, $cashier_id);
    $salesQuery->execute();
    $dailyTotal = $salesQuery->get_result()->fetch_assoc()['daily_total'] ?? 0;
    $salesQuery->close();
...
// ===== MONTHLY SALES (COMPLETED ONLY - THIS CASHIER) =====
$currentMonth = date('Y-m');
$monthlySalesQuery = $conn->prepare("
    SELECT SUM(price * quantity * 1.12) AS monthly_total 
    FROM sales 
    WHERE DATE_FORMAT(sale_date, '%Y-%m') = ? AND status = 'Completed' AND cashier_id = ?
");
$monthlySalesQuery->bind_param("si", $currentMonth, $cashier_id);
$monthlySalesQuery->execute();
$monthlySales = $monthlySalesQuery->get_result()->fetch_assoc()['monthly_total'] ?? 0;
$monthlySalesQuery->close();
...
<title>Sugar Baked - Cashier Dashboard</title>
...
<div class="sidebar">
  ...
  <div class="sidebar-nav">
    <a href="cashier_dashboard.php" class="active"><i class="fas fa-chart-pie"></i><span>Dashboard</span></a>
    <a href="pos.php"><i class="fas fa-cash-register"></i><span>POS</span></a>
    <a href="cashier_sales.php"><i class="fas fa-receipt"></i><span>My Sales</span></a>
...
<div class="content py-3"><br>
  ...
  <div class="welcome-header position-relative">
    <i class="fas fa-user-circle cashier-icon"></i>
    <h2>Welcome, Cashier <?php echo htmlspecialchars($cashier_name); ?>!</h2>
  ...
  <div class="row g-4 mb-4">
    ...
    <div class="col-lg-8">
      <div class="chart-container">
        <h5><i class="fas fa-chart-line text-primary"></i> My Daily Sales (Last 7 Days)</h5>
    ...
    <div class="col-lg-4">
      <div class="chart-container">
        <h5><i class="fas fa-calendar-alt text-success"></i> My Sales This Month</h5>
  ...
  <div class="row g-4">
    ...
    <div class="col-lg-6">
      <div class="table-container">
        <h5><i class="fas fa-trophy text-warning"></i> My Top 5 Products Sold</h5>
        <div class="table-responsive">
          <table class="table table-hover align-middle">
            ...
            <tbody>
              <?php 
              $rank = 1;
              if ($bestsellingResult->num_rows > 0) {
    ...
    <div class="col-lg-6">
      <div class="table-container">
        <h5><i class="fas fa-clock text-info"></i> My Recent Completed Sales</h5>
        <div class="table-responsive">
          <table class="table table-hover align-middle">
            <thead class="table-light">
              <tr>
              ...
              if ($recentSalesResult->num_rows > 0) {
                  while ($row = $recentSalesResult->fetch_assoc()) {
                      echo "<tr>
                              ...
                              <td><small class='text-muted'>" . date('M d, h:i A', strtotime($row['sale_date'])) . "</small></td>
                            </tr>";
                  }
              } else {
