...
// ===== CHECK & CREATE expired_products TABLE IF NOT EXISTS =====
...
if ($checkTable->num_rows == 0) {
    $conn->query("
        CREATE TABLE expired_products (
            ...
            original_expiration_date DATETIME,
            expired_on DATETIME DEFAULT CURRENT_TIMESTAMP,
            INDEX idx_product_id (product_id),
            INDEX idx_expired_on (expired_on)
        ) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4
    ");
}

// ===== AUTO-DEDUCT EXPIRED STOCK FROM PRODUCTS WITH RECORDING =====
...
while ($prod = $expiredProductsQuery->fetch_assoc()) {
    ...
    // Get product details
    $productQuery = $conn->prepare("SELECT stock, product_name, category FROM products WHERE product_id = ?");
    ...
    while ($inv = $expiredInvResult->fetch_assoc()) {
        ...
        if ($deduct_amount > 0) {
            // ===== RECORD TO expired_products TABLE FIRST =====
            $recordExpired = $conn->prepare("
                INSERT INTO expired_products 
                (product_id, product_name, category, quantity_expired, original_expiration_date, expired_on) 
                VALUES (?, ?, ?, ?, ?, NOW())
            ");
            $recordExpired->bind_param("issis", $product_id, $product_name, $category, $deduct_amount, $expiration_date);
            $recordExpired->execute();
            $recordExpired->close();
        ...
        // Mark as expired in inventory BUT KEEP THE STOCK_ADDED VALUE (for tracking)
...
// ===== ADD PRODUCT (Updated with required expiration) =====
...
// ===== UPDATE PRODUCT (Updated with required stock and expiration) =====
...
// Fetch Data (DESC para latest sa taas)
$stmt = $conn->prepare("SELECT * FROM products $where ORDER BY product_id DESC LIMIT ? OFFSET ?");
$params[] = $limit;
$params[] = $offset;
$types .= "ii";
...
// Row number for descending order
...
while($row = $result->fetch_assoc()) {
    ...
    // Edit Modal (Updated with required fields)
    echo "
    ...
    <input type='number' name='stock' class='form-control' value='' min='1' required>
    <small class='text-muted'>Current stock: {$row['stock']} | Must be greater than 0</small>
    </div>
    <div class='mb-2'>
    ...
    <div class='modal-footer'>
    <a href='products.php?delete={$row['product_id']}&page=$page&search=" . urlencode($search) . "' class='btn btn-danger'>Yes, Delete</a>
    <button type='button' class='btn btn-secondary' data-bs-dismiss='modal'>Cancel</button>
    </div>
    </div></div></div>";
    
    $rowNumber--; // Decrement for descending order
}
...
<nav aria-label="Product page navigation" class="text-center mt-3">
  <ul class="pagination justify-content-center flex-wrap my-3">
    ...
    function getStockStatusByPage($conn, $pageNum, $limit, $search = "") {
        ...
        $stmt = $conn->prepare("SELECT stock FROM products $where ORDER BY product_id DESC LIMIT ? OFFSET ?");
        $params[] = $limit;
        $params[] = $offset;
        $types .= "ii";
...
<!-- Add Product Modal (Updated with required expiration) -->
