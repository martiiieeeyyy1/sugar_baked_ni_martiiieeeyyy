<?php
// ===================================================
// sales.php - Sugar Baked Sales Management with Status
// ===================================================
...
// Search
if ($search !== '') {
    $where .= " AND (LOWER(product_name) LIKE ? OR LOWER(cashier_name) LIKE ?)";
    $searchLike = $search . '%';
    $params[] = $searchLike;
    $params[] = $searchLike;
    $types .= 'ss';
...
// ===== Delete Sale (Stay on Correct Page) =====
if (isset($_GET['delete'])) {
    $id = intval($_GET['delete']);
    $page = isset($_GET['page']) ? max(1, intval($_GET['page'])) : 1;

    $stmt = $conn->prepare("SELECT product_name, cashier_name FROM sales WHERE sales_id=?");
    ...
    if ($sale) {
        ...
        if ($stmt->execute()) {
            ...
            $conn->query("UPDATE sales SET sales_id = (@num := @num + 1) ORDER BY sales_id ASC");
            $next = $conn->query("SELECT MAX(sales_id) + 1 AS next_id FROM sales")->fetch_assoc()['next_id'];
            $next = $next ? $next : 1;
            $conn->query("ALTER TABLE sales AUTO_INCREMENT = $next");

            $_SESSION['alert'] = "<div class='alert alert-success alert-dismissible fade show'>
                                    Sale of <b>{$sale['product_name']}</b> by 
                                    <b>{$sale['cashier_name']}</b> successfully deleted!
    ...
    header("Location: sales.php?page=$page");
    exit();
}

// ===== Fetch Sales Data =====
$stmt = $conn->prepare("SELECT * FROM sales $where ORDER BY sales_id ASC LIMIT ? OFFSET ?");
...
.products-table td:nth-child(5) { width: 100px; }
.products-table th:nth-child(6),
.products-table td:nth-child(6) { width: 110px; }
.products-table th:nth-child(7),
.products-table td:nth-child(7) { width: 90px; }
.products-table th:nth-child(8),
.products-table td:nth-child(8) { width: 180px; }
.products-table th:nth-child(9),
.products-table td:nth-child(9) { width: 70px; }
...
<div class="sidebar">
  ...
  <div class="sidebar-nav">
    ...
    <a href="sales.php" class="active"><i class="fas fa-receipt"></i><span>Sales</span></a>
...
<div class="content py-3"><br>
  ...
  <form method="GET" class="mb-3 d-flex align-items-center" style="gap:10px;">
    <input type="text" name="search" class="form-control" placeholder="Search product or cashier..." style="flex:1;"
    ...
    <a href="sales.php" class="btn btn-outline-secondary" style="white-space:nowrap;">Reset</a>
  </form>
...
if ($result->num_rows > 0) {
    while ($row = $result->fetch_assoc()) {
        ...
        $handledBy = htmlspecialchars($row['cashier_name']);
